...
import { sanityFetch, urlFor, POSTS_QUERY, type PostCard } from '@/lib/sanity';
...
export const metadata: Metadata = {
  ...
  description:
    'Learn about image optimization, web performance, and the latest news from the WebPit ecosystem. Convert images to WebP and improve your Core Web Vitals.',
  alternates: { canonical: 'https://webpit.site/blog' },
  openGraph: {
    type: 'website',
    url: 'https://webpit.site/blog',
    title: 'WebPit Blog - Updates & Insights on Image Optimization',
    description:
      'Learn about image optimization, web performance, and the latest news from the WebPit ecosystem.',
    images: ['https://webpit.site/logo.webp'],
  },
  twitter: {
...
export default async function BlogPage() {
  let posts: PostCard[] = [];
  try {
    posts = await sanityFetch<PostCard[]>(POSTS_QUERY, {}, ['post']);
  } catch (error) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 lg:py-20">
      <div className="relative mb-20 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl lg:text-5xl font-extrabold tracking-tight text-slate-900 mb-6 font-sans">
          WebPit Blog
        </h1>
        ...
        >
          <div className="md:w-5/12 w-full p-8 md:p-10 lg:p-12 flex flex-col justify-center order-2 md:order-1">
            <div className="flex flex-wrap items-center gap-3 mb-5">
              <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 font-bold text-[10px] tracking-widest uppercase border border-blue-100 shadow-sm">
                Featured
              </span>
              ...
              <p className="text-sm font-medium text-slate-500">
                {Math.max(2, Math.ceil((featuredPost.rawExcerpt?.length || 500) / 200))} min read
              </p>
            </div>
            ...
            {featuredPost.rawExcerpt && (
              <p className="text-slate-600 text-base line-clamp-3 mb-8 leading-relaxed">{featuredPost.rawExcerpt}</p>
            )}
        ...
        {gridPosts.map((post) => (
          ...
          >
            ...
            <div className="p-8 flex flex-col flex-grow">
              ...
              {post.rawExcerpt && (
                <p className="text-slate-600 line-clamp-3 mb-6 flex-grow leading-relaxed">{post.rawExcerpt}</p>
              )}
              <div className="mt-auto flex items-center text-blue-600 font-bold text-sm select-none">
                Read article
